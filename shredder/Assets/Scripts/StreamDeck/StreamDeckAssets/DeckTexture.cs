...
[CreateAssetMenu(fileName = "New Deck Texture", menuName = "Stream Deck/Deck Texture", order = 0)]
public class DeckTexture : ScriptableObject
{
  ...
  public void GeneratePackets()
  {
    ...
    Image<Bgr24> deckImage = StreamDeckInternal.Texture2DToDeckImage(texture, colourMultiplier);
    List<Image<Bgr24>> buttonImages = StreamDeckInternal.ImageToButtonImages(deckImage, StreamDeck.ButtonCount);

    for (int buttonIndex = 0; buttonIndex < buttonImages.Count; buttonIndex++)
    {
      byte[] jpegTextureData = StreamDeckInternal.ConvertButtonImageToJpegAlloc(buttonImages[buttonIndex]);
      ...
      buttonImages[buttonIndex].Dispose();
      buttonImages[buttonIndex] = null;
    }
...
#if UNITY_EDITOR
[CustomEditor(typeof(DeckTexture))]
public class DeckTextureEditor : Editor
{
  private DeckTexture Target => (DeckTexture)target;
  
  private bool _isDirty = false;
  private DeckTexture _copy;
  ...
  private void CreateCopy()
  {
    if (_copy == null)
    {
      _copy = CreateInstance<DeckTexture>();
    }
